---
title: "lab_10.Rmd"
author: "derek willis"
date: "11/10/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

setwd("/Volumes/Elements/D-Drive/University of Maryland/Classes/Fall 2022/Data Journalism/Github Repository/Data_Journalism_2022_Fall/labs/lab_10/")


```

## You will need

* A Datawrapper account
* Election data in CSV files in the `data` folder

## Load libraries and establish settings

**Task** Create a codeblock and load appropriate packages and settings for this lab.

```{r}
# load libraries here

library(tidyverse)
library(janitor)
library(lubridate)
library(sf)
library(tidycensus)
library(tigris)


```

## Questions

**Q1.** In the previous lab, you made a map showing the difference between the electoral performances of Dan Cox in 2022 and Larry Hogan in 2018. Except a map is probably not the best way to show that. Let's make a chart instead, and the type is up to you, but the goal is to clearly show the differences between the two.

You MUST do the following things when answering this question:
* Load the `md_gov_county.csv` and `md_gov_county_18.csv` files in the data folder
**Done**
* Join those dataframes, specifying the join column. You should add columns to generate percentages for Cox and Hogan.
**Done**
* Export that joined dataframe to a CSV file using `write_csv`.
**Done**
* Using that exported CSV, make a chart with Datawrapper. What kind of chart is up to you, so experiment with a few kinds of charts to see what answers the question above in the clearest way. Your chart MUST:
  * Have a title, description and source that tells readers about the data. The title and description should highlight the most newsworthy or important aspect.
  * Easily readable labels and values
* Publish your chart (you may need to confirm your Datawrapper account if you haven't already in order to do this)
* Put the answer below along with the URL of your chart.

**A1.** 
```{r}
knitr::include_graphics("data/hogan_cox_chart.png")

https://datawrapper.dwcdn.net/6CeFH/1/

```

```{r}
### loading in data
md_gov_county <- read_csv("data/md_gov_county.csv")

md_gov_county_18 <- read_csv("data/md_gov_county_18.csv")

## joining the dataframes
joined_md_gov_counties <- inner_join(md_gov_county, md_gov_county_18, by=c('GEOID'='GEOID'))

# creating the total_county column and the cox and hogan percentage 
joined_md_gov_counties <- joined_md_gov_counties %>%
clean_names() %>% 
mutate(total_county = cox+moore+lashar+wallace+harding+write_ins_x)

joined_md_gov_counties <- joined_md_gov_counties %>% 
mutate(cox_perc = cox/total_county*100) %>% 
mutate(hogan_perc = hogan/total_county*100)

joined_md_gov_counties <- joined_md_gov_counties %>% 
mutate(perc_diff = hogan_perc-cox_perc)

## exporting the new dataframe to a CSV
write_csv(joined_md_gov_counties, "data/joined_md_gov_counties.csv")


```

**Q2.** You're interested in seeing how the methods of voting have changed in Maryland since 2018. Using the `md_votes_by_type.csv` that has data from the top of the ticket (governor in 2018 and 2022, president in 2020)
**Done**

make a chart showing the percentage of votes cast in each category - early voting, election day and mail - over the three years.
**Done**

You'll need to make those percentages in R and then export the CSV using `write_csv`, then make the chart in Datawrapper.
**Done**

Have a title, description and source that tells readers about the data. The title and description should highlight the most newsworthy or important aspect.

HINT: In Datawrapper's "Check and Describe" tab, you can click on a column to get the option of "hiding" it in any visualization.

What do you think the story here is? Put the answer below along with the URL of your chart.

**A2.** The story here is we saw election day voting and mail-in ballots essentially swap in 2018 and 2020. We can likely attribute that to the pandemic. 

```{r}
#loading data
md_votes_by_type <- read_csv("data/md_votes_by_type.csv")

#cleaning data
md_votes_by_type <- md_votes_by_type %>%
clean_names()

#make a chart showing the percentage of votes cast in each category - early voting, election day and mail - over the three years.

write_csv(md_votes_by_type, "data/md_votes_by_type.csv")

# View the chart
knitr::include_graphics("data/votingperc.png")


```


**Q3.** You want to know the geography of ActBlue donors who live in Maryland, so load in the `md_actblue_zip.csv` file in the data folder and calculate a grand total of the totals for each zip code in the dataframe.

Using that total, calculate a percentage of the total for each zip code (you can hard code that number). The export the dataframe using `write_csv` and make a choropleth map in Datawrapper. You will need to:

* Search for Maryland zip codes (use the 2020 data) as the base map

* Upload the CSV file

* Change the tooltip to show both the amount and the percentage (you can put them side by side or put one under the other). Make sure to properly format both numbers to make them readable.

* Have a title, description and source that tells readers about the data. The title and description should highlight the most newsworthy or important aspect.

What do you think the story here is, and what would you want to know more about? Put the answer below along with the URL of your chart.

**A3.** 

**The story here is that some Maryland zipcodes in PG county it seems like have given a ton of money to ActBlue. This is an outlire among other counties and zip codes because the rest haven't given much at all. Another possible angle to this is the fact that so little was given. 

```{r}
#loading data
md_actblue_zip <- read_csv("data/md_actblue_zip.csv")

#calculate a grand total of the totals for each zip code in the dataframe.
## grand total is 20328513

md_actblue_zip %>% 
summarize(total_zip = sum(total))

#Using that total, calculate a percentage of the total for each zip code

md_actblue_zip <- md_actblue_zip %>% 
mutate(zip_perc = (total/20328513)*100)

#write_csv
write_csv(md_actblue_zip, "data/md_actblue_zip.csv")

# View the map
knitr::include_graphics("data/actbluemap.png")

```
