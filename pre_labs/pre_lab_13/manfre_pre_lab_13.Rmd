...
#setwd("/Volumes/Elements/D-Drive/University of Maryland/Classes/Fall 2022/Data Journalism/Github Repository/Data_Journalism_2022_Fall/pre_labs/pre_lab_13.Rmd/")
```
...
#This will make table into a dataframe.
...
**Answer** The columns appear to be bunched together and some are unpopulated. 
...
**Answer** 2, 3 and 4 are referring to the column position.

#gsub means substitute a column with nothing and can use it with just mutate.
...
**Answer** We only see 20 of them when there are 57 li tags. 

#supplying the tag in quotes.
...
**Answer** The output here is that we see all 57 and we see just the names of the committees. It strips away the rest of the HTML.
...
**Answer** It puts that list in 10b into a tibble but it doesnt look perfect because there are a bunch of spaces. 

```{r}
committees <- cmte_results %>% 
  html_elements('li') %>% 
  html_text() %>% 
  as_tibble()
```
...
#this is saying reject anyone that begins with /n
