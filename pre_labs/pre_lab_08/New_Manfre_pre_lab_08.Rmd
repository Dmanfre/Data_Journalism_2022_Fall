...
setwd("/Volumes/Elements/D-Drive/University of Maryland/Classes/Fall 2022/Data Journalism/Github Repository/Data_Journalism_2022_Fall/Pre_labs/Pre_lab_08/")
...
**Answer** This table has 25 rows and 20 columns.
...
**Answer** This shows the top 10 counties with the biggest change in registered voters from 2020 to 2022 and we are then saving it to a new variable so we can only see the output of this tibble. 
...
### This shows the top 10 counties with the biggest change in registered voters.
...
**Answer** We see a box here that is blank. 
...
**Answer** This shows the graph in alphabetical order and not ordered from greatest to least.
...
### geom_bar essentially means "draw me"
...
**Answer** We see the ordered from greatest to least now but it also shows the variable name in the title on the bottom. The other code block did not have this present. This code also has the word reorder. saying x=reorder(enter column names here)
...
**Answer** This will flip it. The counties will go on the Y axis and the bars will go out. This occurs with the verb coord_flip. 
...
**Answer** This gets rid of the gray background from the graph before. 
...
**Answer** This syle is now in the style of the economist and adds a light blue background. 
...
**Answer** This mainly changes the x and y lables (labs for shrot) and allows you to add a caption. 
...
**Answer** We see the amount of contributions via Win Red made on each date from 4/1/22 to June 30 2022.
...
**Answer** We see a basic line chart that does not have a ton of labeling. The graph puts the date on the x axis and the number of contribs on the y axis line. 
...
## Start with data frame, then do ggplot and then do the geometry by naming the x and y axis.

##(aes( means how will you display your data. 
...
**Answer** We see dates along the x axis of the graph via the %b and %d labeling. 
...
**Answer** The text appears on an angle and has a bit more room to breathe.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
