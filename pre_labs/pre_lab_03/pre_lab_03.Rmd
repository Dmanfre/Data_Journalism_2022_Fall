...
setwd("/Volumes/Elements/D-Drive/University of Maryland/Classes/Fall 2022/Data Journalism/Github Repository/Data_Journalism_2022_Fall/pre_labs/pre_lab_03/")
```
...
**Answer** Based on the first 10 rows, the 12th column of row 28450 shows the number 20. 
...
**Answer** I do not see any errors.
...
**Task** Run the following codeblock to glimpse the data.  What data type is the "distrcit" field?  What data type is the "precinct" field? What data type is the mall column? Answer below.

**Answer**  The district field is a numeral, precinct field is a character field and the mall column is a numeral field.
```{r}
...
**Answer**: When mail is NA, there are 402,345 rows. There were 71,589 rows where there were zero mail votes. The implication there is that some Texas precincts may not have reported their mail-in ballots.
...
**Answer** This data says that there wee 12 people that registered to vote in 1900 based on the first 20 results. 
```{r}
yadkin_voters %>% 
...
**Answer** There are some worthy explanations that appear such as moved from country, confirmation pending. They're valid because they are still able to be on the ballot but just have not been removed or verified yet. Overseas citizen also is a valid reason too. 
...
**Answer** The amounts are a character field and it is also mispelled. 
```{r}
glimpse(conowingo)
```
...
**Answer** There are inconsistencies. Lane and Ln are used and there are also unnecessary columns that don't benefit the data analysis. There is a 1 in front of linenumber heading and there shouldn't be numerals in the headings. 
...
**Answer** The headers were made lower case and a x was put in front of the first column but it did not change the fact that there were mispellings of the word Conowingo.
...
**Answer** The error appears in the sumarize function and in sum when it says invalid type. (character). I believe what this means is that there is a character field instead of a numeral field and that the in this set, amount may be a chracter field instead of a number.
...
**Answer** <dbl> is the code for amount when before it said <chr>
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer** The only column to come up is total_amount and it shows 226 contributions.
```{r}
# cleaning function
total_conowingo <- cleaned_conowingo %>%
  summarise(total_amount = sum(amount))
...
**Answer** There are two duplicates. The Donor name is Derrick Hamilton. Great code name.
...
**Answer** The code block cleans up the names and then renames linenumber and then makes the amount column a numeric value and then distinct gets rid of the duplicates. There are 13 row now. 
...
**Answer** The zip code is the name 5-digit number instead of 9 but there are still 13 rows. 

Note: I do not know what start=1L, end=5L means. I don't think this is explained anywhere.
...
**Answer** I do not see the word Arnold in this data set. But all the Cs in the City column are capitalized. But there are two misspellings of Conwingo.
...
**Answer** I still see one misspelling of Conwingo This code has everything we did before.
...
**Answer** There are zero mispellings of Conowingo. What this code does is that it shows distinct rows and makes sure that there are no mispellings. str_detect(city, and the mispelled word)
