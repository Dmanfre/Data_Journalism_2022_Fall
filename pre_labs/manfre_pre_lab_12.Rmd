...
#setwd("/Volumes/Elements/D-Drive/University of Maryland/Classes/Fall 2022/Data Journalism/Github Repository/Data_Journalism_2022_Fall/pre_labs/pre_lab_12.Rmd/")
...
**Answer** We see two tables here. One of the new registration by party and another showing the removals. 
...
**Answer** The result here is a table similar to what we would get on Tabula after we clean it all up. We see new registrations for the month of October 2022.
...
**Task** Run the following code and describe the result.
**Answer** Here we see a table showing the removals from each party and method by which they were removed in october 2022.
...
**Answer** I think that over 50% of new registrations for democrats came on the same day. It gives an impression there weren't many registered before.
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** 61% of democrats were removed because of a criminal conviction. 
...
**Answer**new_registrations_2022_06
...
**Task** Run the following code. How many rows do you have compared to either the October or June dataframes?
**Answer** Now there are 17 rows but I'm a little unsure of what changed.
...
**Answer** They are all relatively small numbers and some even negative

```{r}
new_reg_oct_jun <- new_reg_oct_jun %>% 
  mutate(pct_dem = DEM/total *100, 
         pct_rep = REP/total *100, 
         pct_una = UNA/total *100) %>% 
  mutate(pct_dem06 = DEM_06/total_06 *100, 
         pct_rep06 = REP_06/total_06 *100, 
         pct_una06 = UNA_06/total_06 *100) %>%
...
**Answer** The trend here is that ERIC to By Mail are 60% or higher indicating a majority.

I'm honestly unsure of how to analyse this. Can I have some guidance?
...
**Answer** The ERIC reports increased a lot from June to October as well as votes by mail. THe online registrations seem to stay the same.
