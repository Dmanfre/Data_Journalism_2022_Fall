...
setwd("/Volumes/Elements/D-Drive/University of Maryland/Classes/Fall 2022/Data Journalism/Github Repository/Data_Journalism_2022_Fall/Pre_labs/Pre_lab_09/")
...
**Answer** We see the precinct ID and hwo many congressmen and legislators represent each precinct. 

```{r}
pg_precincts <- st_read("~/Downloads/Election_Precinct_2022_Py.gdb")
...
**Answer** We see a bunch of small precincts that are clumped together. 
...
**Answer** We see data of counties sorted by Name, classification and the geometry 
```{r}
...
**Answer** We see Maryland's counties and various ways to identify it for each county. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We see a map of Maryland with Latatude and Longitudes on the x and y axies. 
...
**Answer** We see precincts of PG County 
...
**Answer** We see a dataset of each precinct and its turnout data.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** We bring the precinct data over to the turnout set to get the total number of eligible voters in each precinct. 
...
**Answer** Here we see the map of the county with total eligable voters and we see it broken down by how many people are there and those are shown in different colors of blue.
...
**Answer** This code just loads the API key.
...
**Answer** We see the list of eligible voters in each county by party in Maryland for this election cycle. 
...
**Answer** Something to see here is that it says ", Maryland" in NAME and the estimate of people. 
...
**Answer** This is geospacial geometry. 
...
**Answer** We see the same dataset as before with county and eligible voters. 
...
**Answer** This removes all of the words called Maryland after a comma and the word county. 
...
**Answer** We are seeing who has the largest voters per capita. 
...
**Answer** This table changes the name St. Mary's to Saint Mary's and Baltimore to Baltimore County. 
...
**Answer** Here we are doing math and we see a joined table. We are getting the per capita totals. 
...
**Answer** In this map we see Maryland and see a breakdown of voters per 10k peple in each county. 
...
**Answer** THis map is a bit more colorful and we can clearly see the defined scale of each county and a much more clear breakdown of eligible voters per 10,000 people. 
...
**Answer** We see in the middle part of Maryland there are more eligible voters and the western portion of Maryland stands pretty much in the middle of the spectrum. 
...
**Answer** In this table we see the unafiliated voters percentage per capita and there are a bit over 1,000 people within the first 10 results.
...
**Answer**  There are some counties wiht an extreamly low UNA voters with fewer than 900 voters. 
