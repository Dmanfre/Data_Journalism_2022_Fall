...
perez_state_data %>% 
Filter(contributor_state != "Maryland")
...
The answer I got was $2,018,222 instead of $2.2 million in 2021. We cleaned up the data pretty well honestly which was great. I'm unsure of what caused us to be off though. Maybe we filtered for incorrect dates but we are very close to what Stephen got which is great!
...
king_john_reciepts <- read_csv("data/king_john_reciepts.csv")

### cleaning names
king_john_reciepts <- king_john_reciepts %>% 
clean_names()

### making the dates column correct
king_john_reciepts <- king_john_reciepts %>% 
mutate(contribution_date=mdy(contribution_date))
...
### Getting rid of x13 and office column
king_john_reciepts <- king_john_reciepts %>% 
select(-x13, -office)

### Filtering to show 2021 only
king_2021 <- king_john_reciepts %>%
filter(between(contribution_date, as.Date('2021-01-01'), as.Date('2021-12-31')))

king_john_reciepts %>% 
filter(year(contribution_date) == 2021) %>% 
summarize(total_2021 = sum(contribution_amount))

### showing his campaign funding
...
We found that both Moore and Perez got over $100,000 from Illinois. 


```{r}
### King Data

### Sowing King's recipets from Illinois
king_john_reciepts %>% 
filter(contributor_state == "Illinois") %>% 
group_by(contributor_state) %>% 
summarize(sum_Ill = sum(contribution_amount))

### Showing Moore reciepts from Illinois
wes_moore_data <- clean_dates

wes_moore_data <- wes_moore_data %>% 
select(-x13)
...
### Moore got 160,075 from Illinois
### King got 157,659 from Illinois
